...
Matrix Matrix::modi(string function)
{
	...
	Matrix temp;
	temp.ary = new float* [row];
	for (int i = 0; i < row; i++)
	{
		temp.ary[i] = new float[col];
		for (int j = 0; j < col; j++)
		{
			if (function.compare("sigmoid"))
			{
				temp.ary[i][j] = 1 / (1 + exp(ary[i][j]));
				cout << temp.ary[i][j] << endl; 
			}
...
int main()
{
	...
	Matrix product = array.dot(test2); 
	cout << "first: " << endl;
	array.print();
	cout << "weights: " << endl; 
	test2.print(); 
	cout << "Prodcut: " << endl; 
	product.print(); 
	cout << "Product: sigmoid: " << endl; 
	Matrix temp = product.modi("sigmoid"); 
	temp.print(); 
	return 0;
}
